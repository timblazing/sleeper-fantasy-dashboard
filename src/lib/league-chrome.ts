...
export type LeagueChrome = { id: string; name: string; season: string; type: string; isDynasty: boolean; basis: ValueBasis; isSuperflex: boolean; matchupWeek: number; avatar: string | null };
...
export async function getLeagueChrome(leagueId: string, source: LeagueSource = liveSource): Promise<LeagueChrome> {
  try {
    ...
    return { id: leagueId, name: league.name, season: league.season, type: format.typeLabel, isDynasty: format.isDynasty, basis: format.basis, isSuperflex: format.superflex, matchupWeek, avatar: league.avatar };
  } catch {
    ...
    return { id: leagueId, name: "League unavailable", season: "", type: "Redraft", isDynasty: false, basis: "redraft", isSuperflex: false, matchupWeek: 1, avatar: null };
  }
}
